"""Read-only local capacity client for Claude."""
...
def read_capacity(home: Path, url: str | None = None) -> dict:
    ...
    try:
        ...
        if not isinstance(snapshot, dict):
            raise OSError("capacity service unavailable; start claude-burndown serve") from None
        ...
        for pool in snapshot.get("pools", []):
            for window in pool.get("windows", []):
                ...
                window["freshness"] = (
                    "unknown" if window.get("used_pct") is None else "fresh" if deadline and now < deadline else "stale"
                )
