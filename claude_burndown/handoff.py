...
def _str(d: dict, keys: tuple[str, ...]) -> str:
    for key in keys:
        if isinstance(d.get(key), (str, int)):
            return str(d[key])
    return ""
...
def _atomic(path: Path, value: dict) -> None:
    ...
    if len(raw.encode()) > _MAX_BYTES:
        raise ValueError("handoff payload exceeds max size")
...
def capture(payload_text: str, home: Path) -> bool:
    """Capture a rate-limit observation from Claude Code statusline stdin.
    
    Redraws with unchanged quota figures preserve the original observation timestamp.
    """
    try:
        ...
        account = _str(payload, ("organization_id", "organizationId", "org_id", "account_id")) or "enterprise-local"
        scope = "org-" + hashlib.sha256(account.encode()).hexdigest()[:16]
        ...
        target = directory / f"{scope}-{digest}.json"
        try:
            old = json.loads(target.read_text(encoding="utf-8"))
        except (OSError, ValueError):
            old = {}
        if old.get("account_scope") == scope and old.get("digest") == digest:
            return False
        ...
        for index, path in enumerate(files):
            try:
                if index >= _MAX_SESSIONS or time.time() - path.stat().st_mtime > _MAX_AGE:
                    path.unlink()
            except OSError:
                pass
...
def read(home: Path) -> list[dict[str, Any]]:
    try:
        paths = sorted(_directory(home).glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)[:_MAX_SESSIONS]
    except OSError:
        return []
    result = []
    for path in paths:
...
def drain(home: Path) -> list[dict[str, Any]]:
    return read(home)


def handoff_directory(home: Path) -> Path:
    return _directory(home)
