"""Burndown math: where you are versus a linear burn of Claude's rate limit budget."""
...
def same_window(a: datetime, b: datetime) -> bool:
    """Nearby reset times describe one allowance window."""
...
def sort_key(bd: Burndown) -> tuple:
    return (bd.window_min, bd.window)
    ...
    for key, items in by_key.items():
        ...
        for sample in items:
            if instances and same_window(sample.resets_at, instances[-1].resets_at):
                ...
                instances.append(
                    WindowInstance(key, sample.provider, sample.window, sample.window_min, sample.resets_at, [sample])
                )
